...
{
    public MediaType[] ReadMediaTypes()
    {
    ...
    private static MediaType[] DeserializeFrom(System.IO.Stream stream)
    {
        stream.Seek(0, SeekOrigin.Begin);
        return Deserialize<Dictionary<string, MimeType>>(stream, Web)?
            .Select(x => new MediaType(x.Value.Mime, x.Key, x.Value.Signs))
